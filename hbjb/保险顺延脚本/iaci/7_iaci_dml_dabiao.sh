...
#参数名以下划线开始，以下部分开发人员可自行修改，并可以添加需要的参数
 ...
 ##本保单的投保确认码 加上标志 和地市
PolicyConfirmNo=`db2 -x  "select PolicyConfirmNo from IACMain_NCPB where flag = '' and citycode = '${_CityCode}'"`
...
do
	...
	do
		...
		db2 "update IACMain_NCP set  Flag = '1' where PolicyConfirmNo = 
			'${B_PolicyConfirmNo}'"	
	...
	StopTravelType2=`db2 -x  "select StopTravelType from IACStopRecover where PolicyConfirmNo = '${B_PolicyConfirmNo}'"`
	...
	then 
		...
		then
			...
			db2 "update IACMain_NCP set  Flag = '1' where PolicyConfirmNo = 
			'${B_PolicyConfirmNo}'"
			PolicyConfirmNoForX5=`db2 -x  "select PolicyConfirmNo from IACMain_NCPX where LastPoliConfirmNo = '${B_PolicyConfirmNo}'"`
			#保存旧的分隔符
			OLD_IFS="$IFS"
			#分隔符设置成空格
			IFS=" "
			array10=($PolicyConfirmNoForX5)
			#变成原来的分隔符
			IFS="$OLD_IFS"
			for PolicyConfirmNoForY5 in ${array10[@]}
			do
				db2 "update IACMain_NCP set  Flag = '1' where PolicyConfirmNo = 
				'${PolicyConfirmNoForY5}'"
			done
	...
	do
		...
		echo "X_StopTravelType:${X_StopTravelType}"	
		
		
		X_StopTravelType1=`db2 -x  "select StopTravelType from IACStopRecover where PolicyConfirmNo = '${B_PolicyConfirmNo}'"`
		X_StopTravelType1=`echo ${X_StopTravelType1} | tr -d ' '`
	    echo "X_StopTravelType1:${X_StopTravelType1}"	
	...
	#需要顺延的数据
	PolicyNo=`db2 -x  "select PolicyNo from IAClaimPolicy_NCP where PolicyConfirmNo = '${B_PolicyConfirmNo}'"`
	if [ -n "${PolicyNo}" ]
	then
		db2 "update IACMain_NCPB set Reason = '', Desc='',
			Flag = '' where PolicyConfirmNo = '${B_PolicyConfirmNo}'"
		db2 "update IACMain_NCP set  Flag = '' where PolicyConfirmNo = 
			'${B_PolicyConfirmNo}'"
	fi
	
	
	#更新基础表中的续保单
	Desc =PolicyNo=`db2 -x  "select Desc from IACMain_NCPB where PolicyConfirmNo = '${B_PolicyConfirmNo}'"`	
	Desc=`echo ${Desc} | tr -d ' '`
	if [ -n "${Desc}" ]
	then
		PolicyConfirmNoForX1=`db2 -x  "select PolicyConfirmNo from IACMain_NCPX where LastPoliConfirmNo = '${B_PolicyConfirmNo}'"`
		#保存旧的分隔符
		OLD_IFS="$IFS"
		#分隔符设置成空格
		IFS=" "
		array6=($PolicyConfirmNoForX)
		#变成原来的分隔符
		IFS="$OLD_IFS"
		for PolicyConfirmNoForY1 in ${array6[@]}
		do
			db2 "update IACMain_NCP set  Flag = '1' where PolicyConfirmNo = 
			'${PolicyConfirmNoForY1}'"
		done
...
times=`date +%s`
main | tee -a Log/7-dml_${times}.log
