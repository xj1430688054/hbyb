...
db2 "insert into IACMAIN_NCPB(POLICYCONFIRMNO, POLICYNO, COMPANYCODE, CITYCODE, STARTDATE, ENDDATE, FRAMENO, LICENSENO, ENGINENO, BUSINESSTYPE, REASON, DESC, FLAG, INPUTDATE)
	select 
		...
		where
			a.startDate <= '${_CLOSINGDATA}' and a.flag='' and a.citycode = '${_CITYCODE}' and
			( b.Flag = '1'  or 
			(b.PolicyConfirmNo = '' or b.PolicyConfirmNo is null))
...
ShortPolicyConfirmNo=`db2 -x  "select PolicyConfirmNo from IACMain_NCPB where citycode = '${_CITYCODE}' and flag = '' and (((
								values days(date(endDate))- days(date(startDate))) <= 30 and (values hour(endDate)) = 0) or 
								((values days(date(endDate))- days(date(startDate))) < 30 and (values hour(endDate)) > 0))"`
...
do
	EndorseConfirmNo=`db2 -x  "select EndorseConfirmNo from IAPHEAD where  PolicyConfirmNo = '${Endor_PolicyConfirmNo}' and EndorseType = '2'"`
	EndorseDate=`db2 -x  "select EndDate from IACMain_NCP where  PolicyConfirmNo = '${Endor_PolicyConfirmNo}' and EndorseType = '2'"`
