...
echo "请输入保险公司不延保保单名称(txt 格式  例： noyanbao)"
	read NOYANAO
...
db2 "LOAD CLIENT FROM  ${NOYANAO}.txt  OF DEL MODIFIED BY DUMPFILE=${DATAPATH}${NOYANAO}.txt NOCHARDEL CODEPAGE=1208 COLDEL| METHOD  P (1,2,3,4,5,6,7,8,9) 
     MESSAGES  ${NOYANAO}.log insert INTO IAPolicyNoPost_NCP"
...
db2 "update IAClaimPolicy_NCP set InputDate = CURRENT TIMESTAMP where InputDate is null"
