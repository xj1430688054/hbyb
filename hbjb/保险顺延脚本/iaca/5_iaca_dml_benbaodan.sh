...
db2 "insert into CACMain_NCPB ( CONFIRMSEQUENCENO, POLICYNO, COMPANYCODE, CITYCODE, EFFECTIVEDATE, EXPIREDATE, VIN, LICENSENO, ENGINENO, BUSINESSTYPE, REASON, DESC, FLAG, INPUTDATE)
		...
		where a.EFFECTIVEDATE <= '${_CLOSINGDATA}'	and a.citycode = '${_CITYCODE}'								
			and a.Flag = '' 
			and (   (b.CONFIRMSEQUENCENO is null or  b.CONFIRMSEQUENCENO='')
					or   b.flag = '1' 					   
				)
			fetch first ${_ROWS} row only
...
shortconfirmsequences=`db2 -x "select 
							...
							where b.flag = '' 
								...
								and (((values days(date(b.ExpireDate))- days(date(b.EffectiveDate)) ) <= 30 and (values hour(b.ExpireDate)) = 0) 
									 or ((values days(date(b.ExpireDate))- days(date(b.EffectiveDate)) ) < 30 and (values hour(b.ExpireDate)) > 0)
									)							
							"`
...
do	
	...
	endorpolicy=`db2 -x "select AmendConfirmNo  from caphead where ConfirmSequenceNo  = '${endor_PolicyConfirmNo}' order by ValidDate desc fetch first 1 rows only"`
	validate=`db2 -x "select ValidDate   from caphead where ConfirmSequenceNo  = '${endor_PolicyConfirmNo}' order by ValidDate desc fetch first 1 rows only"`
...
times=`date +%s`
main | tee -a Log/5_iaca_dml_benbaodan_${times}.log
