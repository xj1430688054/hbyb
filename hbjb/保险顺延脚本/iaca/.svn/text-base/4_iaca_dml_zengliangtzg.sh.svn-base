...
select 
   ...
   c5.CONFIRMSEQUENCENO
		...
		and c4.Flag = ''
			union
		select c1.CONFIRMSEQUENCENO from CAPHEAD c1 inner join CACMain_NCP c2 on c1.CONFIRMSEQUENCENO = c2.CONFIRMSEQUENCENO 
		where c1.CONFIRMDATE > '${_LASTDATE}' and c2.FLAG != '1')c7,
	CACMAIN c5 inner join CACVEHICLE c6 on c5.CONFIRMSEQUENCENO = c6.CONFIRMSEQUENCENO
...
echo "更新打标"
db2 "
MERGE into CACMain_NCP a using (
        select c1.CONFIRMSEQUENCENO from 
                CACMAIN c1 inner join CACMAIN c2 on c1.LASTPOLICYCONFIRMNO = c2.CONFIRMSEQUENCENO 
        where c1.CONFIRMDATE > '${_LASTDATE}' and c2.UNDERWRITEREASON = '1'
 ) e 
        on a.CONFIRMSEQUENCENO = e.CONFIRMSEQUENCENO
when MATCHed then 
update SET a.Flag = '1'
"
################请按照需求书写sql####################
